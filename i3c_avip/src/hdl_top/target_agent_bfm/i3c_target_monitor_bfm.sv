...
interface i3c_target_monitor_bfm(input pclk, 
  ...
  // ──────────────────────────────────────────────
  // DAA internal constants
  // ──────────────────────────────────────────────
  // ENTDAA CCC broadcast address byte: 7'h7E + W(0) = 8'hFC
  localparam logic [7:0] BCAST_7E_W  = 8'hFC;
  // ENTDAA CCC code
  localparam logic [7:0] ENTDAA_CODE = 8'h07;
  // Broadcast 7'h7E + R(1) = 8'hFD
  localparam logic [7:0] BCAST_7E_R  = 8'hFD;
  // Total arbitration bits: 48 PID + 8 BCR + 8 DCR = 64
  ...
  // ──────────────────────────────────────────────
  // NEW: Top-level DAA monitor task
    //   START → 7E+W → ENTDAA → REP_START →
  //   7E+R  → ARB_BITS(64) → ASSIGN →
  //   [LOOP → REP_START → … | STOP]
  // ──────────────────────────────────────────────
  ...
  task sample_daa(output i3c_target_tx daa_txn_q[$]);
    ...
    // ACK from devices (open-drain, pulled low)
    sample_ack(addr_ack);
    ...
    while(more_devices) begin
      ...
      // Repeated START ─────────────────
      detect_start();   // detect_start() handles both START and Sr
      ...
      // ACK/NACK after 7E+R (open-drain arbitration winner pulls low)
      sample_ack(addr_ack);
      if(addr_ack !== ACK) begin
      ...
      // ARB_BITS – sample 64 SDA bits on SCL rise ──
      `uvm_info(name, "DAA: sampling 64 ARB bits {PID,BCR,DCR}", UVM_MEDIUM)
      sample_arb_bits(arb_shift);
 
      // Unpack per FSM: arb_shift[63:16]=PID, [15:8]=BCR, [7:0]=DCR
      ...
      //  ASSIGN – controller writes {dyn_addr, parity} ──
      ...
      // Verify odd parity: parity bit = ~^dyn_addr
      ...
      // ACK from device accepting the assigned address
      sample_ack(addr_ack);
      ...
      // ── Step 7: LOOP – check if another device waits ──
      // The FSM goes LOOP→REP_START when !nack, or LOOP→STOP when nack.
      // On the bus we see either another repeated START (more devices)
      // or a STOP condition.
  ...
  // ──────────────────────────────────────────────
  // NEW: sample one full byte MSB-first on SCL rising edges
  // ──────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────
  // NEW: sample 64 arbitration bits (PID+BCR+DCR)
  //        on every SCL rising edge – mirrors ARB_BITS
  //        state in the FSM (arb_mode=1, SDA released)
  // ──────────────────────────────────────────────
  ...
  // ──────────────────────────────────────────────
  // NEW: after ASSIGN, look at the bus to decide
  //        whether the controller issued a STOP
  //        (no more devices) or a repeated START
  //        (more devices to address).
  //
  //        STOP  : SDA rises while SCL is high
  //        Sr    : SDA falls while SCL is high
  // ──────────────────────────────────────────────
  ...
  task detect_next_daa_condition(output bit more_devices);
    ...
    forever begin
      ...
      // STOP: SDA 0→1 while SCL stays high
